...
SLAMtimeList = zeros(1,100);
SLAMposList = zeros(3,100);
UWBtimeList = zeros(1,80);
UWBposList = zeros(3,80);
SLAMtime = zeros(1,100);
SLAMpos = zeros(100,3);
SLAMposInit = 0;
...
while(length(UWBMsg{1})>=k0(1) || length(UWBMsg{2})>=k0(2) || length(UWBMsg{3})>=k0(3) || length(UWBMsg{4})>=k0(4) || length(IMUMsg)>=k0(5) || length(gps_data_d)>=k0(6) || size(IMGpos,1)>=k0(7)   )
        ...
        if (SLAMposInit(1) == 0)
            ...
            if (length(tempB)~=0) && (length(tempC)~=0)
                ...
                if ((Lf - Ls) > 3) && (length(tempB)>3)
                    ...
                    [absTempDU maxI]= (max(sum(TempDU.^2)));

                    if sqrt(absTempDU) > 2
                        sA = TempDU*transpose(TempDS)*inv(TempDS*transpose(TempDS));
                        % sA = TempDU(:,maxI)*transpose(TempDS(:,maxI))*inv(TempDS(:,maxI)*transpose(TempDS(:,maxI)));
                        s = det(sA);
                        A = sA/s;
                        SLAMposInit = UWBposList(:,Lf)-sA*SLAMposInterp(:,end);
                        MaxabsTempDU = absTempDU;
            ...
            if sqrt(mean(abs(TempOut-PosSLAMout(:,ggd-1)).^2))<0.5
    ...
    elseif (PosHUWBtime(ggd)~=0) && (PosHUWB(ggd)~=0)
        ...
        % if SLAMposList(1,end-5)~=0 && UWBposList(1,end-5)~=0
        %     [tempB] = find(SLAMtimeList~=0);
        %     Ls = min(find(SLAMtimeList(tempB(1)) < UWBtimeList));
        %     Lf = max(find(SLAMtimeList(tempB(end)) > UWBtimeList));


            % if Lf - Ls > 5
            %     SLAMposInterp(1,:) = interp1(SLAMtimeList(Ls:Lf),SLAMposList(1,Ls:Lf),UWBtimeList);
            %     SLAMposInterp(2,:) = interp1(SLAMtimeList(Ls:Lf),SLAMposList(2,Ls:Lf),UWBtimeList);
            %     SLAMposInterp(3,:) = interp1(SLAMtimeList(Ls:Lf),SLAMposList(3,Ls:Lf),UWBtimeList);
            %     % SLAMposInterp(1,:) = interp1(SLAMtimeList,SLAMposList(1,:),UWBtimeList,"cubic","extrap");
            %     % SLAMposInterp(2,:) = interp1(SLAMtimeList,SLAMposList(2,:),UWBtimeList,"cubic","extrap");
            %     % SLAMposInterp(3,:) = interp1(SLAMtimeList,SLAMposList(3,:),UWBtimeList,"cubic","extrap");
            %     TempDU = (UWBposList(1,Ls+1:Lf)+j*UWBposList(2,Ls+1:Lf))-(UWBposList(1,Ls:Lf-1)+j*UWBposList(2,Ls:Lf-1));
            %     TempDS = (SLAMposInterp(1,Ls+1:Lf)+j*SLAMposInterp(3,Ls+1:Lf))-(SLAMposInterp(1,Ls:Lf-1)+j*SLAMposInterp(3,Ls:Lf-1));
            % end
            % 
            % if (SLAMposInit(1)==0) && ( (Lf-Ls) > 3)
            %     % sA = TempDU(1:3,:)*transpose(TempDS(1:3,:))*inv(TempDS(1:3,:)*transpose(TempDS(1:3,:)));
            %     sAc = mean(TempDU./TempDS);
            %     SLAMposN = sAc*(SLAMposInterp(1,Ls:Lf)+j*SLAMposInterp(3,Ls:Lf));
            %     SLAMposInitComp = mean((UWBposList(1,Ls:Lf)+j*UWBposList(2,Ls:Lf))-SLAMposN);
            %     s = abs(sAc);
            %     Am = sAc/s;
            %     SLAMposInit = [real(SLAMposInitComp); imag(SLAMposInitComp); 0];
            %     sA = [real(sAc) -imag(sAc) 0;imag(sAc) real(sAc) 0; 0 0 s];
            % 
            % else
            %     ReSLAMpos(1:3,ggd) = sA*SLAMposInterp([1 3 2],Lf)+SLAMposInit;
            %     if (mean(sqrt(abs(ReSLAMpos(1,ggd)+j*ReSLAMpos(2,ggd)-PosHUWB(ggd)).^2)) > 2) %&& ((UWBtimeList(end)-UWBtimeList(end-1))<10e-3)
            %         SLAMtimeList = zeros(1,100);
            %         SLAMposList = zeros(3,100);
            %         SLAMposInit = 0;
            %         sA = eye(3);
            ...
            %         sA = [real(sAc) -imag(sAc) 0;imag(sAc) real(sAc) 0; 0 0 s];
            % 
            %         % SLAMposInit = SLAMposInit*0.5 + SLAMposInitT*0.5;
            %     end
            % 
            % end
        % end
